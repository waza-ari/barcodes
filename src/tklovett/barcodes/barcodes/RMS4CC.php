 ...
 * KIX is a slightly modified version used by the Royal TNT Post
 * @package tklovett\barcodes\barcodes
 * @author dinesh
 * @author tklovett <tklovett@gmail.com>
 */
class RMS4CC extends OneDimensionalBarcode
{

    /**
     * Construct an RMS4CC barcode array
     ...
     */
    function __construct($code, $kix = false)
    {
        // bar mode
        // 1 = pos 1, length 2
        // 2 = pos 1, length 3
        ...
        $len = strlen($code);
        $this->barcode_array = array('code' => $code, 'maxw' => 0, 'maxh' => 3, 'bcode' => array());
        if (!$kix) {
        ...
        if (!$kix) {
            // stop bar
            $this->barcode_array['bcode'][$k++] = array('t' => 1, 'w' => 1, 'h' => 3, 'p' => 0);
