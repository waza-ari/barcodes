 ...
 */
class EanUpc extends OneDimensionalBarcode
{
    /**
     * Construct an EAN or UPC barcode array
     * @param string $code The code to be represented by this barcode.
     * @param int $length Barcode type: 6 = UPC-E, 8 = EAN8, 13 = EAN13, 12 = UPC-A
     */
    function __construct($code, $length = 2)
    {
        $upce = false;
        if ($length == 6) {
            $length = 12; // UPC-A
        ...
        if ($upce) {
            $this->barcode_array = array('code' => $upce_code, 'maxw' => 0, 'maxh' => 1, 'bcode' => array());
